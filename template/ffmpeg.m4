...
RUN wget -qO - ${FFMPEG_REPO} | tar xz && mv FFmpeg-${FFMPEG_VER} FFmpeg && \
    cd FFmpeg && \
    wget -qO - ${FFMPEG_FLV_PATCH_REPO} | patch -p1 && \
    wget -qO - ${FFMPEG_1TN_PATCH_REPO} | patch -p1 && \
    wget -qO - ${FFMPEG_THREAD_PATCH_REPO} | patch -p1 ifelse(ifelse(index(DOCKER_IMAGE,dev),-1,'false','true'), ifelse(index(DOCKER_IMAGE,dldt),-1,'false','true'),;, && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_01} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_02} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_03} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_04} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_05} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_06} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_07} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_08} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_09} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_10} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_11} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_12} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_13} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_14} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_15} | patch -p1 && \
    wget -qO - ${FFMPEG_MA_PATCH_REPO_16} | patch -p1;
)dnl
